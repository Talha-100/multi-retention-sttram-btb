...
if [ "$#" -ne 8 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [btb] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core]"
    exit 1
fi
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${BTB}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core"
