...
extern uint64_t sttram_refreshes;
extern uint64_t sttram_evictions;
extern uint64_t sttram_write_penalty;
...
struct BTB {
    ...
    void invalidate_entry(uint64_t ip) {
        int idx = index(ip);
        uint64_t tag = get_tag(ip);
        for (uint32_t i = 0; i < theBTB[idx].size(); i++) {
            if (theBTB[idx][i].tag == tag) {
                theBTB[idx].erase(theBTB[idx].begin() + i);
                return;
            }
        }
    }
...
#ifdef VOLATILE_POLICY_REFRESH
                        sttram_refreshes += (lifetime / RETENTION_CYCLES);
#endif
...
void O3_CPU::initialize_btb() {
  ...
  sttram_write_penalty = WRITE_PENALTY;
...
BTB_outcome O3_CPU::btb_prediction(uint64_t ip, uint8_t branch_type) {
  ...
  for (int i = 0; i < NUM_BTB_PARTITIONS; i++) {
	  ...
	  if (btb_entry) {
#ifdef VOLATILE_POLICY_WB
          if ((current_core_cycle[cpu] - btb_entry->write_timestamp) > RETENTION_CYCLES) {
              sttram_evictions++;
              btb_entry = NULL; // Treat as a BTB miss
              break;
          }
#endif
          btb_entry->hit_count++;
...
#ifdef VOLATILE_POLICY_REFRESH
    for (int p = 0; p < NUM_BTB_PARTITIONS; p++) {
        for (uint32_t s = 0; s < btb_partition[p].numSets; s++) {
            for (uint32_t w = 0; w < btb_partition[p].theBTB[s].size(); w++) {
                uint64_t lifetime = current_core_cycle[cpu] - btb_partition[p].theBTB[s][w].write_timestamp;
                sttram_refreshes += (lifetime / RETENTION_CYCLES);
            }
        }
    }
